...
RESULTS_FOLDER = 'results/WhatsApp'
DATASET = 'WhatsApp/whats_br'
c = ModelOCC(chunk_size_false=12, chunk_size_true=12)
...
pd.Series(f1score_scores_ecoocc_features).to_pickle('results/WhatsApp/f1scores_whatsapp_features_ecoocc.pkl')
print("k = {0} \u00B1{1}".format(np.mean(clf.ks), np.std(clf.ks)))
with open('results/WhatsApp/ks_whatsapp_features.pkl', 'wb') as f:
...
pd.Series(f1score_scores_nbpc_features).to_pickle('results/WhatsApp/f1scores_whatsapp_features_nbpc.pkl')
print()
...
results_outliers.to_pickle('results/WhatsApp/whatsapp_features_dcdistance_griddict.pkl')
pd.Series(f1score_scores_dcdistance_features).to_pickle('results/WhatsApp/f1scores_whatsapp_features_dcdistance.pkl')
print()
...
pd.Series(f1score_scores_svm_features).to_pickle('results/WhatsApp/f1scores_whatsapp_features_svm.pkl')
print()
...
f.savefig("{0}/f1score_whatsapp_features.svg".format(RESULTS_FOLDER), dpi=100, bbox_inches='tight')
f.clear()
